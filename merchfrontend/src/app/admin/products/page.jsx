...
export default function AdminProducts() {
  ...
  const removeImageSlot = (index) => {
    if (formData.images.length === 1)
      return CustomToast("You must have at least one image.");
  ...
  const addVariant = (e) => {
    ...
    if (!newVariant.sku || !newVariant.price)
      return CustomToast("SKU and Price are required for variants");
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.category_id || !formData.tag_id)
      return CustomToast("Please select a Category and a Tag");

    // Filter out empty image URLs
    const validImages = formData.images.filter((img) => img.url.trim() !== "");
    if (validImages.length === 0)
      return CustomToast("Please add at least one valid image URL");
    ...
    try {
      ...
      if (res.ok) {
        CustomToast("Product Created Successfully!");
        setFormData(initialFormState);
        setShowForm(false);
        fetchAllData();
      } else {
        CustomToast(data.message || "Failed to create product");
      }
    } catch (e) {
      console.error(e);
      CustomToast("Error submitting form");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Add items, manage stock, and update prices.
            </p>
          </div>
          <button
        ...
        {showForm && (
          <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 mb-10 animate-in fade-in slide-in-from-top-4">
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                  <ImageIcon size={20} className="text-purple-600" /> Product
                  Images
                </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((p) => {
            ...
            const coverImage =
              p.ProductImage?.find((img) => img.is_primary)?.image_url ||
