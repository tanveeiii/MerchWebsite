...
export default function AdminCoupons() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        CustomToast("Coupon Created Successfully!");
        setFormData(initialFormState); // Reset form
        setShowForm(false); // Close form
        fetchCoupons(); // Refresh list
      } else {
        CustomToast(responseData.message || "Failed to create coupon");
      }
    } catch (e) {
      console.error(e);
      CustomToast("An error occurred while creating the coupon.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const deleteCoupon = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        CustomToast("Failed to delete coupon");
      }
    } catch (error) {
        ...
        {showForm && (
          <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 mb-10 animate-in fade-in slide-in-from-top-4">
            ...
            >
              ...
              <button
                disabled={submitting}
                className="col-span-1 md:col-span-2 bg-blue-600 text-white py-4 rounded-lg font-bold hover:bg-blue-700 transition shadow-md disabled:opacity-70 mt-4"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {coupons.length === 0 ? (
            ...
            coupons.map((c) => (
              <div
                key={c.coupon_id}
                className={`p-6 rounded-xl border relative transition-all hover:shadow-md ${
                  c.is_active
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div
                    className={`font-mono font-bold px-3 py-1 rounded text-sm tracking-wide border ${
                      c.is_active
