...
export default function AdminReturnRequests() {
  ...
  const fetchRequests = async () => {
    try {
      const res = await fetch("http://localhost:5000/api/return-request/pending");
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/return-request/${action}/${id}`, {
        method: "PUT"
      });
      if (res.ok) {
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="p-4 font-bold text-sm text-gray-600">Order ID</th>
            ...
            <tbody className="divide-y">
              {requests.map(req => (
                <tr key={req.return_request_id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4 text-sm font-bold text-red-600">${req.refund_amount}</td>
