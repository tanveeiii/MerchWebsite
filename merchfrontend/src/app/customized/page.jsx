...
const CustomizedPage = () => {
  ...
  const handleTransform = (property, change) => {
    ...
    setCustomizations((prev) => {
      ...
      if (property === 'x' || property === 'y') newValue = Math.max(0, Math.min(100, newValue));
      return {
        ...prev,
        [activeSide]: {
          ...prev[activeSide],
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        setCustomizations(prev => ({
          ...
          [activeSide]: {
            ...prev[activeSide],
            graphic: { ...prev[activeSide].graphic, content: event.target.result }
          }
        }));
      };
  ...
  const handleAddToCart = async () => {
    ...
    try {
        ...
        // Success
        alert("Customized Shirt added to cart!");
        router.push('/cart');
  ...
  const renderTextEditor = () => (
    <div className="flex flex-col gap-4">
      <div>
        <label htmlFor="customText" className="font-medium text-gray-700 block mb-2">Enter your text:</label>
        <textarea id="customText" value={customizations[activeSide].text.content} onChange={handleTextChange} maxLength={50} rows={3} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Your Text Here" />
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label htmlFor="fontSelect" className="font-medium text-gray-700 block mb-2">Font:</label>
          <select id="fontSelect" value={customizations[activeSide].text.font} onChange={handleFontChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div>
          ...
          <input id="textColor" type="color" value={customizations[activeSide].text.color} onChange={handleTextColorChange} className="w-full h-10 px-1 py-1 border border-gray-300 rounded-lg cursor-pointer" />
        </div>
      </div>
    </div>
  );
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 items-center justify-items-center gap-2">
          <div /><button onClick={() => handleTransform('y', -5)} className="p-2 bg-gray-100 rounded-md hover:bg-gray-200"><ArrowUp /></button><div />
          ...
          <div /><button onClick={() => handleTransform('y', 5)} className="p-2 bg-gray-100 rounded-md hover:bg-gray-200"><ArrowDown /></button><div />
        </div>
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="container mx-auto max-w-7xl p-4 lg:p-8">
        <h1 className="text-3xl font-bold text-center text-gray-800 mb-6">Create Your Custom T-Shirt</h1>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl shadow-xl p-8 flex flex-col items-center justify-center min-h-[85vh]">
            ...
            <div className="relative w-full max-w-lg">
              {activeSide === "front" ? <TShirtFrontSvg colors={shirtColors} config={shirtConfig} /> : <TShirtBackSvg colors={shirtColors} config={shirtConfig} />}
          ...
          <div className="lg:col-span-1 bg-white rounded-2xl shadow-xl p-6 flex flex-col">
            ...
            {activeTab !== "color" && activeTab !== "style" && (<div className="border-t pt-6">{renderTransformControls()}</div>)}
            ...
            <div className="border-t pt-6 space-y-4 mt-auto">
              ...
              >
                {isSubmitting ? <><Loader2 className="animate-spin" /> Adding...</> : "Add to Cart"}
              </button>
              <button onClick={resetCustomization} className="w-full flex items-center justify-center gap-2 bg-gray-200 text-gray-700 font-medium py-3 px-6 rounded-lg hover:bg-gray-300 transition-colors">
...
const TabButton = ({ icon, label, isActive, onClick }) => (
  <button onClick={onClick} className={`flex-1 flex flex-col items-center justify-center gap-1 px-4 py-3 font-medium transition-all ${isActive ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-500 hover:text-gray-800 hover:bg-gray-50 rounded-t-lg"}`}>{React.cloneElement(icon, { className: 'w-5 h-5' })}<span className="text-sm">{label}</span></button>
);
